...
/*   Updated: 2023/10/09 11:56:56 by alaparic         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_handle_was(enum e_keys key, t_game *game)
{
	...
	if (key == W || key == UP)
	{
		if (game->map_data.map[(int)game->player.y][(int)(game->player.x + game->player.direction->x * 10)] != '1')
			game->player.x += game->player.direction->x;
		if (game->map_data.map[(int)(game->player.y + game->player.direction->y * 10)][(int)game->player.x] != '1')
			game->player.y += game->player.direction->y;
	}
	else if (key == A)
	{
		if (game->map_data.map[(int)game->player.y][(int)(game->player.x + game->player.direction->y * 10)] != '1')
			game->player.x += game->player.direction->y;
		if (game->map_data.map[(int)(game->player.y - game->player.direction->x * 10)][(int)game->player.x] != '1')
